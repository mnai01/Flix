...
const PosterContent: React.FC<SelectedMediaProps> = ({
    title,
    runtime,
    runtimeNative,
    release_date,
    ...
    return (
        <Box height={'100%'} width={'100%'} position={'relative'}>
            <Box zIndex={1} padding={50} position={'absolute'} style={{ color: '#FFFFFF' }} bottom={'25%'} width={'35%'}>
                ...
                <Flex mb={25} gap={15}>
                    <Text fontWeight={500}>{runtimeNative ? runtimeNative + ' Seasons' : convertMinsToHrsMins(runtime)}</Text>
                    <Text>|</Text>
                    <Text fontWeight={500}>{release_date}</Text>
                </Flex>

...
const Poster: React.FC<SelectedMediaProps> = (posterProps) => {
    const [load, setLoad] = useState(false);
    const { loading, backdrop_path, ...posterContent } = posterProps;

    useEffect(() => {
        setLoad(backdrop_path === undefined);
    }, [backdrop_path]);

    // REMOVE MOVIE POSTER IF MOVIE MISSING
            ...
            {loading ? (
                ...
                <Box zIndex={0} position={'absolute'} style={{ color: '#FFFFFF' }} margin={'auto'} top={0} bottom={0} p={0} width={'100%'} height={'100%'}>
                    ...
                    <Image
                        ...
                        fallbackSrc={'https://s28504.pcdn.co/wp-content/themes/idology/images/video-place-holder.png'}
                        onLoad={() => setLoad(true)}
                        display={load ? 'block' : 'none'}
                    />
                </Box>
